...
public class DataSourceConfiguredState extends MediaRecorderStates{

	TaintAnalyzer ta;
	private SymbolSpace localSymSpace = Config.getInstance().getLocalSymbolSpace();
	
	public DataSourceConfiguredState(TaintAnalyzer taParam)
	{
		this.ta = taParam;
	}
	
	public DataSourceConfiguredState(){}
	...
	public State update(MediaPrepareEvent e) {
		...
		if(recorderEntry != null){
			...
			if(currState != null && currState instanceof DataSourceConfiguredState){
				State state = new PreparedState(this.ta);
