...
public class CipherInputStreamDefinedState extends FileReadingStates {

	TaintAnalyzer ta;
	private SymbolSpace localSymSpace = Config.getInstance().getLocalSymbolSpace();

	public CipherInputStreamDefinedState(TaintAnalyzer taParam) {
		this.ta = taParam;
	}

	public CipherInputStreamDefinedState() {
	}
	...
	public State update(CipherInputStreamReadEvent e) {
		...
		if (callerEntry != null) {
			...
			if (state != null && state instanceof CipherInputStreamDefinedState) {
