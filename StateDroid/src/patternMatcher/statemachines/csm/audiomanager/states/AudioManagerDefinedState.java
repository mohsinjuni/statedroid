...
import patternMatcher.statemachines.csm.audiomanager.AudioManagerStates;
...
public class AudioManagerDefinedState extends AudioManagerStates {

	TaintAnalyzer ta;
	private SymbolSpace localSymSpace = Config.getInstance().getLocalSymbolSpace();

	public AudioManagerDefinedState(TaintAnalyzer taParam) {
		this.ta = taParam;
	}

	public AudioManagerDefinedState() {
	}
		...
		if (audioMgrEntry != null) {
			...
			if (recordFieldList != null && recordFieldList.size() > 0) {
				...
				if (!ringerModeValue.isEmpty()) {
					...
					String msg = "";
					int ringerMode = -1;
					if (ringerModeValue.contains("1") || ringerModeValue.contains("2") || ringerModeValue.contains("existing")) {
						ringerMode = 1;
