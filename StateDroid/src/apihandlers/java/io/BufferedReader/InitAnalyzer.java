...
//		0x12 invoke-direct v6, v9, Ljava/io/BufferedReader;-><init>(Ljava/io/Reader;)V    //This reader could be an InputStreamReader or FileReader
		...
		if(regCount == 2)
		{
	        ...
	        if(callerEntry != null)
	        {
	        	...
	        	if(inputParamEntry != null)
	        	{
	        		...
	        		recordFieldList.put("fileReaderEntry", inputParamEntry);
	        		callerEntry.getEntryDetails().setRecordFieldList(recordFieldList);
	        		callerEntry.getEntryDetails().setRecord(true);	        	

	        		EventFactory.getInstance().registerEvent("bufferedReaderDefinedEvent", new BufferedReaderDefinedEvent());
					Event event = EventFactory.getInstance().createEvent("bufferedReaderDefinedEvent");
					event.setName("bufferedReaderDefinedEvent");
