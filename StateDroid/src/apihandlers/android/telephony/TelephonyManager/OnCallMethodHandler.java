...
public class OnCallMethodHandler {
	...
	public OnCallMethodHandler(APK appPkg, InstructionResponse currIR, SymbolSpace symbolSpace, ContextStack contextStack) {
		this.apk = appPkg;
		this.ir = currIR;
	...
	public OnCallMethodHandler(TaintAnalyzer ta) {
		this.apk = ta.getApk();
		this.ir = ta.getIr();
	...
	public void saveParamEntries(CFG cfg) {
		...
		if (involvedRegisters.size() > 0) {
			for (int i = involvedRegCount - 1; i > 1; i--) {
			...
			Register callerReg = involvedRegisters.get(1);
			SymbolTableEntry entry = localSymSpace.find(callerReg.getName());
			if (entry != null) {
				SymbolTableEntry newEntry = (SymbolTableEntry) entry.clone(); // shallow
																				// copy
				String newName = String.valueOf("v").concat(String.valueOf(maxRegCount - 1));
